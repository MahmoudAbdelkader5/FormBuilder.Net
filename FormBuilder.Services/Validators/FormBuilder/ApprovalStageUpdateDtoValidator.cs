...
{
    public class ApprovalStageUpdateDtoValidator : AbstractValidator<ApprovalStageUpdateDto>
    {
        public ApprovalStageUpdateDtoValidator()
        {
            RuleFor(x => x.StageName)
                .Length(2, 200)
                .WithMessage("Stage name must be between 2 and 200 characters")
                .When(x => !string.IsNullOrEmpty(x.StageName));

            RuleFor(x => x.StageOrder)
                .GreaterThan(0)
                .WithMessage("Stage order must be greater than 0")
                .When(x => x.StageOrder.HasValue);
